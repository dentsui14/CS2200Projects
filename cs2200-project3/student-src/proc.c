...
void proc_init(pcb_t *proc) {
    ...
    pfn_t physical_frame_number = free_frame();
    memset(mem + (physical_frame_number * PAGE_SIZE), 0, PAGE_SIZE);
    // pte_t* page_table = (pte_t *) (mem + (page_table * PAGE_SIZE));
    frame_table[physical_frame_number].protected = 1;
    frame_table[physical_frame_number].mapped = 0;
    frame_table[physical_frame_number].process = proc;
    proc->saved_ptbr = physical_frame_number;
