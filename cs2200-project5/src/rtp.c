...
packet_t *packetize(char *buffer, int length, int *count) {
    ...
    if (length % MAX_PAYLOAD_LENGTH != 0) {
        num_packets++;
        last_size = (size_t) length % MAX_PAYLOAD_LENGTH;
    } else {
        last_size = MAX_PAYLOAD_LENGTH;
    }
    packet_t *packets = malloc(sizeof(packet_t) * (size_t) num_packets);
    ...
    memcpy(packets[num_packets - 1].payload, buffer + ((num_packets - 1) * MAX_PAYLOAD_LENGTH), last_size);
    packets[num_packets - 1].payload_length = last_size;
    packets[num_packets - 1].checksum = checksum(packets[num_packets - 1].payload, last_size);
 ...
 * 
 * Example: "abcd" checksum = (a * 2) + (b * 2) + (c / 2) + (d / 2)
 * 
    ...
    do {
        ...
        do {
            ...
            if (packet.type == LAST_DATA || packet.type == DATA) {
                packet_t* return_packet = malloc(sizeof(packet_t));
                if (checksum(packet.payload, packet.payload_length) == packet.checksum) {
                    return_packet->type = ACK;
                    if (buffer_length == 0) {
                        buffer = malloc((size_t) packet.payload_length);
                    } else {
                        buffer = realloc(buffer, (size_t) (packet.payload_length + buffer_length));
                    }
