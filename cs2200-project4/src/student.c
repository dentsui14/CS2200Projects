...
{
    pcb_t* new_pcb = dequeue(rq);
    if (new_pcb != NULL) {
        new_pcb->state = PROCESS_RUNNING;
    }
    pthread_mutex_lock(&current_mutex);
    current[cpu_id] = new_pcb;
    pthread_mutex_unlock(&current_mutex);
    context_switch(cpu_id, new_pcb, timeslice);
}
...
extern void preempt(unsigned int cpu_id)
{
    ...
    pthread_mutex_lock(&current_mutex);
    pcb_t* current_running = current[cpu_id];
    current_running->state = PROCESS_READY;
    enqueue(rq, current_running);
    pthread_mutex_unlock(&current_mutex);
    schedule(cpu_id);
}
...
extern void yield(unsigned int cpu_id)
{
    pthread_mutex_lock(&current_mutex);
    pcb_t* current_running = current[cpu_id];
    current_running->state = PROCESS_WAITING;
    pthread_mutex_unlock(&current_mutex);
    schedule(cpu_id);
...
extern void terminate(unsigned int cpu_id)
{
    pthread_mutex_lock(&current_mutex);
    pcb_t* current_running = current[cpu_id];
    current_running->state = PROCESS_TERMINATED;
    pthread_mutex_unlock(&current_mutex);
    schedule(cpu_id);
}
